    ...
    glitter.dataBase.init("publicBeans", function () {
        glitter.serialUtil.getObject("publicBeans", "publicBeans", function (result) {
            ...
            glitter.runJsInterFace("FileManager_CheckFileExists",{
                    ...
                    glitter.runJsInterFace("DataBase_InitByAssets",{
                        ...
                        if(response.result){
                            glitter.runJsInterFace("DataBase_Query",
                                {string: `select ${'`'}${glitter.publicBeans.language}${'`'},id from language `, name: "language"},
                                function (response) {
                                    if (response.result) {
                ...
                if(result){loadFile()}else{loadLocal()}
            })
...
glitter.selectFunction={
    ...
    switchToPage:function (){
        ...
        if(glitter.mmyInterFace.selectMMY["Relearn code"].indexOf('INDIRECT')!==-1&&(glitter.selectFunction.select!==glitter.selectFunction.enum.OSOM)){
