...
class ApiRequest {
    constructor() {
        ...
        this.postWithDialog = function (map, success, error) {
            console.log("request:"+JSON.stringify(map))
            glitter.openDiaLog('dialog/Dia_DataLoading.html','Dia_DataLoading',false,false,{})
            $.ajax({
                type: "POST",
                url: glitter.fileModelInterFace.domain+ '/orangeJsonApi',
        ...
        this.postTextWithDialog = function (text, success, error) {
            ...
            $.ajax({
                type: "POST",
                url: "http://bento3.orange-electronic.com/exsql",
                data: text,
                timeout: 1000*30,
                success: function (data) {
                    glitter.closeDiaLogWithTag('Dia_DataLoading')
        ...
        this.postRequest= function (map, success, error) {
            $.ajax({
                type: "POST",
                url: glitter.fileModelInterFace.domain,
                data: '' + JSON.stringify(map),
                timeout: 5000,
                success: function (data) {
                    try {
        ...
        this.getWebResources= function (rout, success, error) {
            $.ajax({
                type: "GET",
                url: rout,
                timeout: 1000*60,
