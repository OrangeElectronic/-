...
class CobsFraming: DataExchangeProtocol
{
    ...
    private var _lockQueue = DispatchQueue(label: "com.onsemi.fota.CobsFraming.lockQueue"/*, attributes: .concurrent*/)
    ...
    private var _cobsFramingQueue = DispatchQueue(label: "com.onsemi.fota.cobsFramingQueue", attributes: .concurrent)
...
//        NSLog("TempPrint: DR CobsFraming.dataReceived 0x\(BinaryString.toString(buffer: args.data.toArray()))")
...
//            NSLog("TempPrint: buffer: 0x\(BinaryString.toString(buffer: data.toArray()))")
            
            //COBS encoding
            let encoded = try Cobs.encode(src: data)
//            NSLog("----- encoded cobs: 0x\(BinaryString.toString(buffer: encoded.toArray()))")
