...
class HdlcFlowControl : DataExchangeProtocol
{
    ...
    private let _sendQueueLock = DispatchQueue(label: "com.onsemi.fota.HdlcFlowControl.sendQueuelock", attributes:  .concurrent)
    ...
    private var _acknowledgeQueueLock = DispatchQueue(label: "com.onsemi.fota.HdlcFlowControl.acknowledgeQueueLock", attributes:  .concurrent)
    
    private var _receiveQueue = Queue<Data>()
    private var _receiveQueueLock = DispatchQueue(label: "com.onsemi.fota.HdlcFlowControl.receiveQueueLock", attributes:  .concurrent)
    ...
    private var _hdlcControlQueue = DispatchQueue(label: "com.onsemi.fota.hdlcControlQueue", attributes: .concurrent)
...
//        NSLog("TempPrint: DR HdlcFlowControl.dataReceived: 0x\(BinaryString.toString(buffer: args.data.toArray()))")
        _receiveQueueLock.sync {
    ...
    private func sendIFrame(data: HdlcTransmitEntry)
    {
        ...
        do
        {
//            NSLog("----- sendIFrame: 0x\(BinaryString.toString(buffer: frame.toArray()))")
...
//        NSLog("TempPrint: frame: \(frame[0])")
    ...
    private func sendSFrame()
    {
        ...
        do
        {
//            NSLog("----- sendSFrame: 0x\(BinaryString.toString(buffer: frame.toArray()))")
            try _lowerDataExcange.transmit(buffer: frame)
        }
        catch
        {
