    ...
    constructor() {
        ...
        this.openQrScanner = function (callback) {
            switch (glitter.deviceType) {
                case appearType.Android:
                    ...
                    glitter.runJsInterFace("openQrScanner",{},function (){  })
                    break;
        ...
        * */
        ...
        this.getGPS = function (callback) {
            glitter.runJsInterFace("GpsManager_getGps",{},function (response){
                if(response.result){
        ...
        * */
        ...
        this.playSound = function (rout) {
            switch (glitter.deviceType) {
                case appearType.Android:
                    glitter.runJsInterFace("SoundManager_Play",{rout:rout},function (response){
                    })
                    break;
        ...
        * */
        ...
        this.chooseMedia = function (type,callback) {
        ...
        this.KeyboardUtil ={
            document:document,
            OnHeightChange:function (height, visibleHeight) {
                const body = glitter.KeyboardUtil.document.body;
                glitter.KeyboardUtil.height = body.clientHeight;
                glitter.KeyboardUtil.visibleHeight = visibleHeight * (body.clientHeight) / height;
                const active = glitter.KeyboardUtil.document.activeElement;

                if (height === visibleHeight) {
                    glitter.KeyboardUtil.SetElementScrollTop(body, 0);
                    return;
                }
                let focusTopRelateWindow = glitter.KeyboardUtil.FindElementPositionRelateToWindow(active);
                ...
                if (bottom > glitter.KeyboardUtil.height)
                    bottom = glitter.KeyboardUtil.height;
                const offset = bottom - glitter.KeyboardUtil.visibleHeight;
                console.log(`focusTopRelateWindow:${focusTopRelateWindow}--bottom:${bottom}--offset:${offset}--clientHeight:${body.clientHeight}`)
                if (offset <= 0) {
                    glitter.KeyboardUtil.SetElementScrollTop(body, 0);
                    return;
                }
                glitter.KeyboardUtil.SetElementScrollTop(body, offset);
            },
            SetElementScrollTop:function (e, offset) {
                console.log(`SetElementScrollTop->e:${e}-offset:${offset}`)
                if (offset === 0) {
                    $(e).css('transform','translateY(0px)');
                }
                else {
                    $(e).css('transform',`translateY(-${offset}px)`);
                }
            },
            FindElementPositionRelateToWindow:function (element) {
                let curtop = 0;
                let curtopscroll = 0;
