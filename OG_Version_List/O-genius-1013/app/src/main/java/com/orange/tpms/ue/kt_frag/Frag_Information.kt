...
class Frag_Information : RootFragement(R.layout.fragment_frag__information) {
    ...
    private fun initHelper() {
        ...
        //请求失败
        informationHelper.setOnFailedRequestListener { `object` ->
            JzActivity.getControlInstance().toast(`object`.toString())
        }
        //锁定列表回调
        informationHelper.setOnGetInformationListener(object :
            InformationHelper.OnGetInformationListener {
            override fun onGetInformation(arrayList: List<InformationBean>) {
                informationAdapter.items = arrayList
                informationAdapter.notifyDataSetChanged()
            }
        })
        //读取锁定列表
        informationHelper.getInformation(JzActivity.getControlInstance().getRootActivity())
    }
