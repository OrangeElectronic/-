...
class Frag_Setting_Unit : RootFragement(R.layout.fragment_frag__setting__unit) {
    override fun viewInit() {
        ...
        initView()
        initHelper()
        if(JzActivity.getControlInstance().getPro("Pre",-1)==-1){
            unitHelper.setPressure(activity!!, 2)
        }
    }
    ...
    private fun initHelper() {
        ...
        //温度单位列表回调
        unitHelper.setOnGetTempListener(object :UnitHelper.OnGetTempListener{
            override fun onGetTemp(select: Int, arrayList: List<String>?) {
        ...
        //压力单位列表回调
        unitHelper.setOnGetPressureListener(object :UnitHelper.OnGetPressureListener{
            override fun onGetPressure(select: Int, arrayList: List<String>?) {
        ...
        //数字单位列表回调
        unitHelper.setOnGetNumeralListener(object :UnitHelper.OnGetNumeralListener {
            override fun onGetNumeral(select: Int, arrayList: List<String>?) {
