...
class BleCommand {
    constructor() {
            ...
            glitter.bleUtil.manager.isConnect(function (result) {
                if (result) {
                    if (glitter.deviceType === glitter.deviceTypeEnum.Android) {
                        glitter.bleUtil.manager.writeHex(data.replaceAll(' ', ''), "00008D81-0000-1000-8000-00805F9B34FB", "00008D82-0000-1000-8000-00805F9B34FB")
                    } else {
                        glitter.bleUtil.manager.writeHex(data.replaceAll(' ', ''), "8D81", "8D82")
                    }
                }
            })
        }
        ...
        * */
        this.readMultiSensor = function (count, callback) {
            glitter.command.cancel = false
            var hex = glitter.mmyInterFace.selectMMY["OBD Product No. (hex)"]
            var lf = glitter.mmyInterFace.selectMMY["LF Power"]
            if (hex.length === 4) {
            ...
            glitter.command.commandStart(function () {
                glitter.mmyInterFace.getS19("text", function (s19text) {
                    ...
                    function checkData(data, place, toNext) {
                        ...
                        let command = glitter.command.toCrc("0A 13 LONG DATA PLACE FF F5".replace(/ /g, "").replace("LONG", long)
        ...
        this.getOBDid = function (callback) {
            glitter.command.cancel = false
            var obdVersion = "60BF00010" + glitter.mmyInterFace.selectMMY.Wheel_Count + "FF0A";
            var clock = Clock()
        ...
        this.writeObd = function (itemArray, callback) {
            ...
            let timer = setInterval(function () {
                ...
                if (rx.indexOf("60B201FFFFFFFFD30A") !== -1) {
                    callback(true)
                    clearInterval(timer)
                }
            }, 200)
        ...
        this.updateFW = function (data, result) {
            ...
            function writeFlush(toNext) {
                ...
                function start() {
                    if (i === long) {
                        ...
                        setTimeout(function (){toNext()},1000)
                        return
                    }
                    var b = i
                    if (b >= 255) {
            ...
            function writeVersion(toNext) {
                ...
                var timer = setInterval(function () {
                    ...
                    if ((rx.length === 14)) {
                        setTimeout(function (){toNext()},200)
                        clearInterval(timer)
                    }
                }, 200)
            }
