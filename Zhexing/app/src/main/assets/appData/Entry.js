...
function onCreate() {
    glitter.share.version = '4.6'
    glitter.bleList = []
    glitter.share.memoryData=[]
        ...
        glitter.addScript("model/UserInfoMation.js", function () {
            glitter.dataBase.initByFile("MMY", "database/1.0.db", function () {
                ...
                glitter.getPro('language',function (response){
                    ...
                    if(sys.indexOf("tw")!==-1||(sys.indexOf("zh")!==-1 && sys.indexOf("zh-rcn")===-1)){  position="tw"}
                    if(response.data){position=['en','zh-rcn','tw'][parseInt(response.data)]}
